...
for later difficulty calibration, which does not block gold generation. H4 v3
incorporates the latest owner feedback and awaits review of eight regenerated
samples. This document is not benchmark data and is not approval for bulk
conversion.
...
| H4 | Programmatic extraction from ACI-Bench notes using header, problem-block, and semantic routing rules | Chloe confirms extraction semantics/audit | Jessica implements extractor | Review regenerated v3 eight-case pack and approve extraction/difficulty rules |
...
The H4 v3 draft uses ACI-Bench's existing source splits and maps them to
development/pilot/validation/test. In addition to the v2 rules, it strips
...
descriptions. The held-out test files remain locked. The specification remains
`draft_pending_approval`; it is not yet authorization for bulk extraction.
...
3. The converter generates an agreed evaluator-only review sample (eight H4 v3
   records in the current review round).
