...
Uses the latest result per (task_id, framework) in results/metrics/*.jsonl,
...
def main():
    for vertical, config in GROUND_TRUTH_CONFIG.items():
        ...
        for (task_id, framework), r in results.items():
            ...
            metrics = evaluate_hallucination_risk(result, ground_truth[task_id], config["answer_key"])
        ...
        for framework, rows in sorted(by_framework.items()):
            ...
            for r in confidently_wrong:
                print(
                    ...
                    f"(confidence={r['stated_confidence']!r}) but ground truth is {r['ground_truth']!r}"
                )
        print()
