...
Pulls every recorded attempt from
...
def _build_run(row: dict) -> dict:
    ...
    return {
        ...
        "result_run_id": row.get("run_id") or row["_dashboard_row_id"],
...
def _load_dashboard_results(vertical: str) -> list[dict]:
    """Keep every result attempt so repeats and retries remain auditable."""
    ...
    with path.open("r", encoding="utf-8") as handle:
        for row_number, line in enumerate(handle, start=1):
            if not line.strip():
                continue
            row = json.loads(line)
            row["_dashboard_row_id"] = f"legacy-row-{row_number}"
            rows.append(row)
    return rows
...
def build_payload(
    ...
    rows = _load_dashboard_results(vertical)
    runs = [
        _build_run(row)
        for row in rows
...
<script>
/* ------------------------------------------------------------------ *
 * DATA -- generated by scripts/generate_dashboard.py from every recorded
 * attempt. The embedded payload is
...
function cellRunKey(caseId, framework, experimentLabel){
  return caseId + "|" + framework + "|" + experimentLabel;
}
function runDomKey(r){ return cellRunKey(r.case_id, r.framework, r.experiment_label) + "|" + r.result_run_id; }
const runByDomKey = {};
const runsByCellKey = {};
DATA.runs.forEach(r => {
  runByDomKey[runDomKey(r)] = r;
  const cellKey = cellRunKey(r.case_id, r.framework, r.experiment_label);
  (runsByCellKey[cellKey] ||= []).push(r);
});
...
function render(){
  ...
  cases.forEach(c => {
    ...
    DATA.frameworks.filter(f=>state.active.has(f.id)).forEach(f => {
      ...
      const cellRuns = runsByCellKey[lookupKey] || [];
      if (!cellRuns.length){
      ...
      cellRuns.forEach(r => {
        const b = bucket(r);
        const domKey = runDomKey(r);
        const attemptLabel = r.repeat != null || r.attempt != null
          ? ` r${r.repeat ?? "?"}/a${r.attempt ?? "?"}` : "";
        html += `<button class="chip ${b}" data-run="${esc(domKey)}" aria-label="${esc(c.id)} ${esc(f.label)}${esc(attemptLabel)} ${esc(r.final_state_verdict)}">
          ${r.schema_valid === false ? '<span class="schema-flag"></span>' : ''}
          <span class="tag">${TAG[b]}${esc(attemptLabel)}</span><span class="code">${esc(r.final_state_verdict)}</span></button>`;
      });
      html += `</td>`;
    });
    html += `</tr>`;
...
function openDrawer(domKey){
  ...
  document.getElementById("drawerHead").innerHTML = `
    ...
    <div class="facts">
      ...
      <span>repeat <b>${r.repeat ?? "n/a"}</b></span>
      <span>attempt <b>${r.attempt ?? "n/a"}</b></span>
      <span>result <b>${esc(r.result_run_id)}</b></span>
